...
import {
  spanishA1VocabularyItems,
  spanishLessons,
  spanishUnits,
} from "../../../plugins/tutor/modules/spanish/lessons/index.ts";
...
const EXPECTED_SPANISH_UNIT_NAMES = ["Pronunciation", "A1 Grammar", "A1 Vocabulary"];
...
describe("Spanish lesson catalog", () => {
  test("registers the three A1 progress units from the same lesson registry", () => {
    expect(spanishUnits.map((unit) => unit.name)).toEqual(EXPECTED_SPANISH_UNIT_NAMES);

    const lessonIds = new Set(spanishLessons.map((lesson) => lesson.id));
    const unitLessonIds = new Set(spanishUnits.flatMap((unit) => unit.lessonIds));
    const orphanLessonIds = [...lessonIds].filter((lessonId) => !unitLessonIds.has(lessonId));
  ...
  test("all lessons are reachable from root lessons through prerequisites", () => {
    ...
    const unreachable = spanishLessons
      .map((lesson) => lesson.id)
      .filter((lessonId) => !reachable.has(lessonId));
  ...
  test("adds 100+ unique A1 vocabulary SRS items", () => {
    const expectedTerms = new Set(spanishA1VocabularyItems.map((item) => item.term));
    const srsTerms = spanishLessons
      .filter((lesson) => lesson.unit === "vocabulary")
      .flatMap((lesson) => lesson.srsItems?.map((item) => item.front) ?? []);

    expect(spanishA1VocabularyItems.length).toBeGreaterThanOrEqual(100);
    expect(srsTerms).toHaveLength(spanishA1VocabularyItems.length);
    expect(new Set(srsTerms)).toEqual(expectedTerms);
  });

  test("generated Spanish production prompts ask for Spanish", () => {
    const productionPrompts = spanishLessons
      .flatMap((lesson) => lesson.exercises)
      .filter((exercise) => exercise.type === "production")
      .map((exercise) => exercise.prompt);
    ...
    expect(productionPrompts.length).toBeGreaterThan(0);
    for (const prompt of productionPrompts) {
      expect(prompt).toContain("Type the Spanish");
      expect(prompt).not.toContain("Type the Japanese");
    }
  });
});
