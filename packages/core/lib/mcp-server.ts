...
export function buildInstructions(ctx: AppContext): string {
  ...
  return [
    ...
    "",
    "## Time & Timezones",
    'If the message has user_timezone="<IANA>" (e.g. "America/New_York"), interpret clock times ("9am", "tomorrow 3pm") in that zone and convert to UTC before calling set_reminder.due_at (format: YYYY-MM-DDTHH:MM:SSZ). Example: user_timezone="America/New_York" + "9am" → "2026-01-15T14:00:00Z".',
    'Relative durations ("in 30 min", "every 2h") are zone-independent — pass through unchanged.',
    "If no user_timezone is set and the user gives a clock time, ask them to run `/timezone value:<IANA>` rather than guessing.",
    "",
    "## Conversation Mode",
