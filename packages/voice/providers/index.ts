...
import type { PluginConfig } from "@choomfie/shared";
...
export async function getSTTProvider(config: PluginConfig): Promise<STTProvider> {
...
export async function getTTSProvider(config: PluginConfig): Promise<TTSProvider> {
  const voiceConfig = config.getVoiceConfig();
