...
function getTwitterConfig(ctx: PluginContext): {
  username: string;
  password: string;
  email: string;
} {
...
export const twitterTools: ToolDef[] = [
  ...
  },
  {
    ...
    handler: async (args, _ctx) => {
      try {
        const client = getTwitterClient();
        const result = await withRetry(() => client.postTweet(args.text as string), {
          label: "twitter_post",
          maxAttempts: 2,
        });
    ...
    definition: {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          image: {
            type: "string",
            description: "Absolute file path to image (PNG/JPG/GIF)",
          },
        },
        required: ["text", "image"],
      },
  ...
  },
  {
    ...
    handler: async (args, _ctx) => {
      try {
        ...
        const summary = results.map((r, i) => `${i + 1}. ${r.url}`).join("\n");
