...
function normalizeLevel(level: string): string {
  ...
  const match = /^HSK([1-6])$/.exec(compact);
  return match ? `HSK ${match[1]}` : level;
}
...
export const chineseModule: TutorModule = {
  ...
  buildTutorPrompt(level: string, ctx?: TutorPromptContext): string {
    ...
    return `You are a Mandarin Chinese tutor. ${LEVEL_GUIDES[levelKey] || LEVEL_GUIDES["HSK 1"]}${pinyinDirective}
