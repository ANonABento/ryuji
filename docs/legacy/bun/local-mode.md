...
- Tool calls are not yet round-tripped through local chat backends. Slash commands and built-in interactions still work; chat replies are LLM text only. Tool-augmented chat is a future task.
...
- Experimental Hermes peer-brain mode can route local replies through a running Hermes API server by setting `local.brainProvider` to `hermes` and `local.hermesUrl` to the server's `/v1` endpoint. See `docs/hermes-decision.md`.
