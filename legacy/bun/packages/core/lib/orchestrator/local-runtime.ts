/**
 * LocalRuntime — orchestrates Choomfie when running fully local (Ollama or an
 * experimental peer agent backend, no Anthropic / no Claude Code MCP). Wires the chat provider, registry,
 * router, idle monitor, and background worker into one always-on loop.
 *
 * Initialization order:
 *   1. ping the configured local backend (fail fast if not running)
...
import { HermesProvider, OllamaProvider, type ChatProvider } from "./chat-provider.ts";
import {
  ModelRegistry,
  type ModelMetadata,
  type ModelSelection,
...
export interface LocalReplyOptions {
  ...
  hints?: RoutingHints;
  /** Backend-specific session hints for peer-agent providers such as Hermes */
  backendSession?: {
    id?: string;
    key?: string;
  };
...
export class LocalRuntime {
  ...
  constructor(public readonly config: LocalRuntimeConfig) {
    this.provider = createChatProvider(config);
    const chatModel =
      config.brainProvider === "hermes"
        ? config.hermesModel || DEFAULT_LOCAL_CONFIG.hermesModel!
        : config.chatModel;
    const codingModel =
      config.brainProvider === "hermes"
        ? config.hermesModel || DEFAULT_LOCAL_CONFIG.hermesModel!
  ...
  /** Verify backend, discover models, prewarm chat + coding, start background worker. */
  async start(): Promise<void> {
    if (this.started) return;
    if (!(await this.provider.ping())) {
      throw new Error(this.unreachableMessage());
    }
  ...
  async reply(userText: string, options: LocalReplyOptions = {}): Promise<LocalReplyResult> {
    ...
    const stream = this.provider.chatStream({
      ...
      messages,
      sessionId: options.backendSession?.id,
      sessionKey: options.backendSession?.key,
  ...
  private unreachableMessage(): string {
    if (this.config.brainProvider === "hermes") {
      return (
        `Hermes API server unreachable at ${this.config.hermesUrl ?? DEFAULT_LOCAL_CONFIG.hermesUrl}. ` +
        "Start Hermes with API_SERVER_ENABLED=true and point local.hermesUrl at its /v1 endpoint."
      );
    }
    return (
...
function createChatProvider(config: LocalRuntimeConfig): ChatProvider {
  if (config.brainProvider === "hermes") {
