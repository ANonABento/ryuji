...
function formatModel(m: ModelMetadata, sel: { chat: string; coding: string }): string {
  ...
  const backend = m.family === "hermes" ? "hermes" : m.speedTier;
  return `\`${m.name}\`${params}${quant} (${backend})${vram}${role}`;
}

function formatProvider(cfg: {
  brainProvider?: "ollama" | "hermes";
  ollamaUrl: string;
  hermesUrl?: string;
}): string {
  if (cfg.brainProvider === "hermes") {
    return `Hermes API @ ${cfg.hermesUrl ?? "http://127.0.0.1:8642/v1"}`;
  }
  return `Ollama @ ${cfg.ollamaUrl}`;
}
...
registerCommand("model", {
  ...
  handler: async (interaction, ctx) => {
    ...
    if (sub === "swap") {
      ...
      if (runtime.provider.kind === "hermes") {
        await runtime.registry.swap(role === "chat" ? "coding" : "chat", name);
      }
      // Persist to config so the swap survives restart.
      const local = ctx.config.getLocalConfig();
      ctx.config.setLocalConfig(
        runtime.provider.kind === "hermes"
          ? { ...local, hermesModel: name }
          : { ...local, ...(role === "chat" ? { chatModel: name } : { codingModel: name }) },
      );
      ...
      await interaction.editReply({
        content:
          runtime.provider.kind === "hermes"
            ? `Swapped Hermes model/profile → \`${name}\`.`
            : `Swapped ${role} model → \`${name}\` (chat=${sel.chat}, coding=${sel.coding}). Pre-warming...`,
      });
      return;
    }
